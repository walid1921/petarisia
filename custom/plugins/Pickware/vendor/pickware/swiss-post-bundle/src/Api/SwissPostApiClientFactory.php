...
class SwissPostApiClientFactory
{
    private const BASE_URL = 'https://dcapi.apis.post.ch/barcode/v1/';

    public function __construct(
        #[Autowire(service: 'monolog.logger.pickware_swiss_post')]
        private readonly LoggerInterface $logger,
        private readonly SwissPostOAuthApiClientFactory $swissPostOAuthApiClientFactory,
    ) {}

    public function createSwissPostApiClient(SwissPostApiClientConfig $config): RestApiClient
    {
        ...
        $handlerStack->unshift(
            new OAuthMiddleware(
                $config,
                $this->swissPostOAuthApiClientFactory->createSwissPostOAuthApiClient(),
            ),
        );
