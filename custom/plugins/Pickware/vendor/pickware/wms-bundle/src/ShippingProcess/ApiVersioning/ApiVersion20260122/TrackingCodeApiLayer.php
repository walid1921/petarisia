...
namespace Pickware\PickwareWms\ShippingProcess\ApiVersioning\ApiVersion20260122;
...
use Pickware\PickwareWms\Delivery\ApiVersioning\ApiVersion20260122\TrackingCodeTransforming;
...
#[EntityApiLayer(entity: ShippingProcessDefinition::ENTITY_NAME, method: 'search')]
class TrackingCodeApiLayer implements ApiLayer
{
    ...
    public function transformResponse(Request $request, Response $response, Context $context): void
    {
        ...
        if ($response->headers->get('Content-Type') === 'application/vnd.api+json') {
            ...
            if (isset($content['data']['pickingProcesses'])) {
                foreach ($content['data']['pickingProcesses'] as &$pickingProcess) {
                    if (isset($pickingProcess['deliveries'])) {
                        self::transformParcelsInDeliveries($pickingProcess['deliveries']);
                    }
                }
                unset($pickingProcess);
            } elseif (isset($content['data'][0])) {
                foreach ($content['data'] as &$shippingProcess) {
                    if (isset($shippingProcess['pickingProcesses']) && is_array($shippingProcess['pickingProcesses'])) {
                        foreach ($shippingProcess['pickingProcesses'] as &$pickingProcess) {
                            if (isset($pickingProcess['deliveries'])) {
                                self::transformParcelsInDeliveries($pickingProcess['deliveries']);
                            }
                        }
