...
class PickingProfileGenerator implements DemodataGeneratorInterface
{
    private const PROFILE_SUFFIXES = [
        'Picking',
        'Orders',
        'Shipping',
        'Delivery',
    ...
    {
        return PickingProfileDefinition::class;
    }
    ...
    public function generate(int $numberOfItems, DemodataContext $demodataContext, array $options = []): void
    {
        ...
        for ($i = 0; $i < $numberOfItems; $i++) {
            $createPayloads[] = [
                'id' => Uuid::randomHex(),
                'name' => $this->generateProfileName($demodataContext),
                'position' => $startPosition + $i,
                'isPartialDeliveryAllowed' => $demodataContext->getFaker()->boolean(),
                'filter' => null,
            ];
        ...
        $demodataContext->getConsole()->text(sprintf(
            'Created %d picking profiles',
            $numberOfItems,
        ));
    }

    private function generateProfileName(DemodataContext $demodataContext): string
    {
        $faker = $demodataContext->getFaker();
        $prefix = $faker->colorName();
        $suffix = $faker->randomElement(self::PROFILE_SUFFIXES);
