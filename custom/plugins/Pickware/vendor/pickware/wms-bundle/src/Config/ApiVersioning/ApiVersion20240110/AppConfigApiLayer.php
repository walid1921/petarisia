...
namespace Pickware\PickwareWms\Config\ApiVersioning\ApiVersion20240110;

use Pickware\ApiVersioningBundle\ApiLayer;
use Pickware\ApiVersioningBundle\ApiVersion;
use Pickware\PhpStandardLibrary\Json\Json;
use Pickware\PickwareWms\ApiVersion\ApiVersion20240110;
...
class AppConfigApiLayer implements ApiLayer
{
    ...
    public function transformResponse(Request $request, Response $response, Context $context): void
    {
        ...
        if (isset($content['featureFlags'])) {
            $content['featureFlags'] = array_keys($content['featureFlags'], true, true);
        }
        if (isset($content['technicalFeatureFlags'])) {
            $content['technicalFeatureFlags'] = array_keys($content['technicalFeatureFlags'], true, true);
        }
