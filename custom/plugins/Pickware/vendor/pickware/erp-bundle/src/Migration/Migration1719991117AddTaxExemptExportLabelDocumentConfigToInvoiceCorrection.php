...
class Migration1719991117AddTaxExemptExportLabelDocumentConfigToInvoiceCorrection extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        $connection->transactional(function(Connection $transaction): void {
            $invoiceCorrectionConfig = $transaction->executeQuery(
                <<<SQL
                    SELECT
                    ...
                    SQL,
                ['technicalName' => 'pickware_erp_invoice_correction'],
            )->fetchAssociative();

            if ($invoiceCorrectionConfig === false) {
                return;
            }

            $config = Json::decodeToArray($invoiceCorrectionConfig['config']);
            ...
            $transaction->executeQuery(
                ...
                [
                    'id' => hex2bin($invoiceCorrectionConfig['id']),
