...
#[EntityApiLayer(entity: CashPointClosingDefinition::ENTITY_NAME, method: 'search')]
class CashPointClosingSearchApiLayer implements ApiLayer
{
    ...
    public function transformResponse(Request $request, Response $response, Context $context): void
    {
        ...
        if ($response->headers->get('Content-Type') === 'application/vnd.api+json') {
            ...
            foreach ($content['data'] as &$cashPointClosing) {
                foreach ($cashPointClosing['cashPointClosingTransactions'] as &$transaction) {
